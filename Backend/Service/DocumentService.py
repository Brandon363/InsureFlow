...
async def upload_document(db_session: Session, file: UploadFile, user_id: int,
    ...
    new_document = DocumentEntity(user_id=user_id, claim_id=claim_id,
        policy_id=policy_id, type=document_type, name=original_filename, url=file_path,
        mime_type=file.content_type or "application/octet-stream", size=file_size, entity_status=EntityStatus.ACTIVE)

    db_session.add(new_document)
    db_session.commit()
    db_session.refresh(new_document)

    return DocumentResponse(status_code=201, success=True, message="Document uploaded successfully", document=new_document)
...
def get_all_active_documents(db_session: Session) -> DocumentResponse:
    ...
    return DocumentResponse(status_code=200, success=True, message="Documents retrieved successfully", documents=documents or [])
...
def create_document(db_session: Session, create_request: DocumentCreate) -> DocumentResponse:
    ...
    new_document = DocumentEntity(user_id=create_request.user_id, claim_id=create_request.claim_id,
        policy_id=create_request.policy_id, type=create_request.type, name=create_request.name,
        url=create_request.url, mime_type=create_request.mime_type, size=create_request.size, entity_status=EntityStatus.ACTIVE)

    db_session.add(new_document)
    db_session.commit()
    db_session.refresh(new_document)

    return DocumentResponse(status_code=201, success=True, message="Document created successfully", document=new_document)
