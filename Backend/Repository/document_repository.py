...
def find_active_documents_by_user_id(db_session: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db_session.query(DocumentEntity).filter(
        DocumentEntity.user_id == user_id, DocumentEntity.entity_status == EntityStatus.ACTIVE).offset(skip).limit(limit).all()


def find_active_documents_by_claim_id(db_session: Session, claim_id: int, skip: int = 0, limit: int = 100):
    return db_session.query(DocumentEntity).filter(
        DocumentEntity.claim_id == claim_id, DocumentEntity.entity_status == EntityStatus.ACTIVE).offset(skip).limit(limit).all()


def find_active_documents_by_type(db_session: Session, document_type: str, skip: int = 0, limit: int = 100):
    return db_session.query(DocumentEntity).filter(
        DocumentEntity.type == document_type, DocumentEntity.entity_status == EntityStatus.ACTIVE).offset(skip).limit(limit).all()


def find_active_documents_by_policy_id(db_session: Session, policy_id: int, skip: int = 0, limit: int = 100):
    return db_session.query(DocumentEntity).filter(
        DocumentEntity.policy_id == policy_id, DocumentEntity.entity_status == EntityStatus.ACTIVE).offset(skip).limit(limit).all()