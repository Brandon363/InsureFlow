...
class VerificationTrackingEntity(Base):
    __tablename__ = "verification_tracking"

    id = Column(Integer, primary_key=True, index=True)
    stage = Column(SQLEnum(VerificationTrackingStage), nullable=False)
    ...
    user = relationship("UserEntity", foreign_keys=[user_id], back_populates="tracked_verification_tracking")
    action_performed_by = relationship("UserEntity", foreign_keys=[action_performed_by_id], back_populates="performed_verification_tracking")